...
import { sidebarLinks, subSideBareLink } from './sdiebarLinks';
...
export default function SideBar() {
  return (
    <aside className="fixed w-full bg-white pr-5 sm:w-fit sm:sticky top-0 h-full z-50">
      <Logo />
      <section className="flex flex-col gap-28">
        ...
        {ROLES === 'patient' && (
          <Link
            ...
            className="btn-rounded  bg-[#1579e5]  flex items-center gap-2 text-white w-fit my-auto mx-auto sm:ml-3 md:ml-3 hover:bg-[#157ae59f] transition-all duration-200"
          >
            <i className={'bi bi-box-arrow-in-right text-md'}></i>
...
function List({ links }: links) {
  const sectionLinks = links.filter((link: link) => {
    return link.permission.includes(ROLES);
  });
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              (isActive
                ...
                : 'text-black before:bg-[#ffffff] ') + 'li-style btn-rounded'
            }
            key={link.title}
          >
...
function Logo() {
  return (
    <section className='flex items-center justify-between sm:px-0 px-4'>
      ...
      <div className="flex gap-2 items-center justify-center m-5 mr-0 ">
        ...
        <h3 className="font-bold text-[#1579e5] text-xl sm:hidden md:flex">
          <span className="text-black">C</span>ureSync
        </h3>
      </div>
    </section>
  );
